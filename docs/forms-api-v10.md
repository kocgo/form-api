...
### Fields
Fields are declared as a typed map to preserve field-name autocompletion. Each field supports visibility, disability, and required conditions. Widgets handle arrays and nested objects directly, so there is only a single field shape.
